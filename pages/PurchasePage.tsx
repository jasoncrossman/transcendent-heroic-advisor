...
const PurchasePage: React.FC = () => {
  ...
  // Checks the URL for a success message from Stripe to trigger the book popup
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Trigger Stripe Embedded Popup
      // @ts-ignore
      ...
      const { error } = await stripe.redirectToCheckout({
        ...
        mode: 'payment',
        // This adds a "flag" to the URL when they return so the Shipping Modal pops up
      ...
      // Fallback redirect if embedded fails
  ...
  const handleShippingSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Sends the shipping address to your same Zapier hook
    await fetch("https://hooks.zapier.com/hooks/catch/18380285/uwqwyud/", {
      method: "POST",
      mode: "no-cors",
      body: JSON.stringify({ email, shippingAddress, type: "Signed Book Shipping" }),
    });
    setShowSuccessModal(false);
    // Finally, send them to the academy
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-16 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {/* SHIPPING ADDRESS POPUP (Shown after successful purchase) */}
      {showSuccessModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2 font-serif">Enrollment Successful!</h2>
